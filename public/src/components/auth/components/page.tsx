...
const Auth: React.FC<SessionInfo> = ({ session  }) => {
    ...
    return (
        <main className="flex items-center h-full ml-5 gap-x-3 lg:w-2/5 lg:justify-end lg:ml-0 ">
            {session && session?.user? (
              <div className='flex items-center gap-x-4'>
                <p className='text-gray-600'>{session?.user.email}</p>
                <button onClick={() => {signOut();}} className="bg-blue-700 hover:bg-blue-900 text-white uppercase text-xs font-semibold p-2 rounded transition-colors duration-300">Salir</button>
              </div>
              ) : (
              <div className='inline-flex gap-x-2'>
                <button onClick={() => openModal('login')} className="bg-green-500 hover:bg-green-700 text-white text-sm font-semibold py-1 px-2 rounded transition-colors duration-300">Ingresar</button>
                <button onClick={() => openModal('singup')} className="bg-blue-700 hover:bg-blue-900 text-white text-sm font-semibold py-1 px-2 rounded transition-colors duration-300">Inscribirse</button>
              </div>
            )}
            {showModal && (
                ...
                <div className="w-1/4 h-3/5 flex justify-between items-center rounded-2xl bg-white shadow-lg">
                  <div className="relative w-full h-full p-6">
                    ...
                    <div className={`flex flex-row w-full items-center ${activeTab === 'auth' ?  'hidden' : ''}`}>
                      <button onClick={() => openModal('login')} className={`text-green-800 rounded-full px-4 py-1 inline-flex text-sm font-semibold transition duration-300 mr-2 ${activeTab === 'login' ?  'bg-green-700 hover:bg-green-800 !text-white' : ''}`}>Ingresar</button>
                      <button onClick={() => openModal('singup')} className={`text-blue-800 rounded-full px-4 py-1 inline-flex text-sm font-semibold transition duration-300 mr-2 ${activeTab === 'singup' ? 'bg-blue-700 hover:bg-blue-800 !text-white' : ''}`}>Inscribirse</button>
                      {showForgotPasswordModal ? (
                        <button onClick={() => openModal('forgot_password_confirm')} className={`text-green-800 rounded-full px-2 py-1 inline-flex text-sm font-semibold transition duration-300 mr-2 ${activeTab === 'forgot_password_confirm' ? 'bg-green-700 hover:bg-green-800' : ''}`}><AiFillUnlock/></button>
                        ) : (
                        <button onClick={() => openModal('forgot-password')} className={`text-yellow-800 rounded-full px-2 py-1 inline-flex text-sm font-semibold transition duration-300 mr-2 ${activeTab === 'forgot-password' ? 'bg-yellow-700 hover:bg-yellow-800 !text-white' : ''}`}><AiFillLock/></button>
                        )}
                    </div>
                    <div style={{ display: activeTab === 'login' ? 'block' : 'none' }} className={`h-full my-4 ${activeTab === 'login' ? 'animate-fade-in animate__animated animate__fadeIn' : 'animate-fade-out animate__animated animate__fadeOut'} ${activeTab === 'signup' ? 'hidden' : ''}`}>
                      ...
                      <div className="text-start items-center inline-flex gap-x-2">
                        ...
                        <button onClick={() => openModal('singup')} className="cursor-pointer text-green-800 hover:text-blue-800 transition-colors duration-300 -mt-1">Inscribete</button>
                      </div><br />
                      <button onClick={() => openModal('forgot-password')} className="hover:underline text-xs text-blue-800 font-semibold">¿Olvidaste la contraseña?</button>
                    </div>
                    <div style={{ display: activeTab === 'singup' ? 'block' : 'none' }} className={`h-full my-4 ${activeTab === 'singup' ? 'animate-fade-in animate__animated animate__fadeIn' : 'animate-fade-out animate__animated animate__fadeOut'} ${activeTab === 'login' ? 'hidden' : ''}`}>
                      ...
                      <div className="inline-flex gap-x-2 items-center">
                        ...
                        <button onClick={() => openModal('login')} className="cursor-pointer text-green-500 hover:text-blue-600 transition-colors duration-300 -mt-1">Ingresar</button>
                      </div>
                    </div>
