...
const ForgotPasswordConfirmModal: React.FC<ForgotPasswordConfirmModal> = ({ closeModal, updateForgotPasswordModalState }) => {
  ...
  return (
    <div>
      ...
      { success && (<div className="text-lime-700 text-xs mt-2">{success}</div>)}
      { error && (<div className="text-red-400 text-xs mt-2">{error}</div>)}
      { !error && !success && (<div className="text-gray-800 text-xs mt-2 h-6">¿Necesitas Ayuda? savelasquezo@gmail.com</div>)}
    </div>
  );
};
